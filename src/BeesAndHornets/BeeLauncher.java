...
public class BeeLauncher extends madkit.kernel.Agent {
    ...
    private ArrayList<AbstractAgent> hornetList = new ArrayList<>();
    ...
    protected void activate() {
        ...
        getLogger().info(() -> "Initial launch time: " + (System.nanoTime() - startTime));

        // Start scheduler and viewer
        ...
        pause(3000); // Allow agents to stabilize before entering live mode
        launchQueens(1);
        launchHornet(1);
    }
	...
	while (isAlive()) {
		...
		if (Math.random() < .8) {
			...
			if (queensList.size() > 1)
				...
				//killBees(true, (int) (Math.random() * 7000) + 100);
			    //else
				//killBees(true, (int) (Math.random() * 2000) + 100);
				if (queensList.size() > 7)
					killBees(true, (int) (Math.random() * 70000) + 1000);
				    else
					killBees(true, (int) (Math.random() * 20000) + 1000);
		    }
		    ...
		    launchBees((int) (Math.random() * 15) + 5);
		    }
		}
		else {
...
//        killBees(false, 150); // Randomly remove some bees
//
//        double random = Math.random();
//        if (random < 0.8) {
//            if (queensList.size() < 10) {
//                launchQueens(1 + (int) (Math.random() * 2));
//            }
//        } else if (random < 0.3) {
//            if (beesList.size() < 200000 && Runtime.getRuntime().freeMemory() > 100000) {
//                launchBees(10 + (int) (Math.random() * 20));
//            }
//        } else {
//            killBees(false, (int) (Math.random() * 500) + 1);
//        }
//    }
    ...
    private void launchBees(int numberOfBees) {
        ...
        List<AbstractAgent> beesBucket = launchAgentBucket(
            Bee.class.getName(),
            numberOfBees,
    ...
    private void launchHornet(int numberOfHornets) {
        ...
        for (int i = 0; i < numberOfHornets; i++) {
            HornetAgent hornet = new HornetAgent();
            launchAgent(hornet, true);
            hornetList.add(hornet);
        }
    }
